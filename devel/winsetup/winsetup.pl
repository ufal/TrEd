...
$ppm->repo_enable($_,0) for $ppm->repos;

# add our local repository
my $my_repo = $ppm->repo_add(
  name => 'tred_install',
  packlist_uri => 'file://'.$package_dir
);
...
chomp @features;
 ...
 my @packages = $ppm->packages_missing
   ( want => [map { [$_,0] } @features] );
 if (@packages) {
...
$ppm->repo_sync;
print STDERR $@ if $@;
$ppm->repo_enable($_,$repo_state{$_})
  for
  grep {$repo_state{$_}}
  keys %repo_state;
