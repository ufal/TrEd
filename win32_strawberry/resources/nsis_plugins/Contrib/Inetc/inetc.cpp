...
//		if((hFile = HttpOpenRequest(hConn, _T("OPTIONS"), path, NULL, NULL, NULL,
			INTERNET_FLAG_RELOAD | INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_NO_COOKIES, 0)) != NULL)
		{
			if(*szAuth)
			{
...
// request itself
	if(status == ST_URLOPEN)
	{
		if((hFile = HttpOpenRequest(hConn, fput ? _T("PUT") : (fhead ? _T("HEAD") : (szPost ? _T("POST") : NULL)),
			path, NULL, NULL, NULL,
			INTERNET_FLAG_RELOAD | INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_NO_COOKIES | 
...
void onTimer(HWND hDlg)
{
	...
	wsprintf(b,  _T("%s - %s"), szCaption, szStatus[status]);
...
BOOL WINAPI dlgProc(HWND hDlg,
	 ...
	 case WM_COMMAND:
		 ...
		 case IDCANCEL:
			 if(nocancel) break;
			 if(szQuestion &&
				 MessageBox(hDlg, szQuestion, szCaption, MB_ICONWARNING|MB_YESNO) == IDNO)
				 break;
			 status = ST_CANCELLED;
		 case IDOK:
...
// for repeating /nounload plug-un calls - global vars clean up
	...
	*szCaption = *szCancel = *szUserAgent = *szBasic = *szAuth = 0;
	...
	while(!popstring(url) && *url == _T('/'))
	{
		...
		else if(lstrcmpi(url, _T("/timeout")) == 0)
		{
			popstring(url);
			timeout = _tcstol(url, NULL, 10);
		}
