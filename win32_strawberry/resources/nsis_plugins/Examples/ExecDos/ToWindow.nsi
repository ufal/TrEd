...
Function CustomPageEdit
  ...
  !insertmacro MUI_INSTALLOPTIONS_EXTRACT ml_edit.ini
  !insertmacro MUI_INSTALLOPTIONS_INITDIALOG ml_edit.ini
  Pop $0
  !insertmacro MUI_INSTALLOPTIONS_READ $R8 ml_edit.ini "Field 1" HWND
  StrCpy $R0 0
  Call enableNext
  StrCpy $R0 1
  GetFunctionAddress $R2 enableNext
  ExecDos::exec /NOUNLOAD /ASYNC /TOWINDOW /ENDFUNC=$R2 "$EXEDIR\consApp.exe" "test_login$\ntest_pwd$\n" $R8
  Pop $R9
...
Function CustomPageList
  ...
  !insertmacro MUI_INSTALLOPTIONS_EXTRACT listbox.ini
  !insertmacro MUI_INSTALLOPTIONS_INITDIALOG listbox.ini
  Pop $0
  !insertmacro MUI_INSTALLOPTIONS_READ $R8 listbox.ini "Field 1" HWND
  StrCpy $R0 0
  Call enableNext
  StrCpy $R0 1
  ExecDos::exec /NOUNLOAD /ASYNC /TOWINDOW /ENDFUNC=$R2 "$EXEDIR\consApp.exe" "test_login$\ntest_pwd$\n" $R8
